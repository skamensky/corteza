  ...
  <div>
    ...
    >
      ...
      >
        ...
        <h5 class="text-primary mb-2">
          {{ $t('chart.configurator.metrics.name') }}
        </h5>
            ...
            >
              <thead>
                <tr class="text-primary">
                  <th>
                    {{ $t('chart.configurator.metrics.field') }}
                  </th>
                  <th style="width: 250px;">
                    {{ $t('chart.configurator.metrics.stack.name') }}
                  </th>
                </tr>
              </thead>
...
export default {
  ...
  methods: {
    updateDataColumns (values) {
      let temp = this.options.dataColumns
      this.options.dataColumns = values

      this.options.dataColumns.forEach((c, idx) => {
        const initialCol = temp.find(({ name }) => name === c.name)

        if (initialCol) {
          this.options.dataColumns[idx] = initialCol
        }
      })
    },
